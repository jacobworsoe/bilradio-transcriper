...
_Last updated: 2026-04-06 (session: single-episode improvement refresh + handover checklist). Operator + maintainer notes._
...
- **Single-episode refresh (re-import same GUID):** use when replacing or polishing an existing improved JSON (sponsor cleanup, timecodes, new bullets). **(1)** Overwrite **`data/transcripts_improved/<stem>.json`** (or delete and recreate). **(2)** Clear SQLite topics for that episode so **`import-bullets`** replaces rows cleanly: `DELETE FROM topic_bullets WHERE episode_guid = '<guid>'`; `DELETE FROM topic_sections WHERE episode_guid = '<guid>'`; `UPDATE episodes SET status = 'transcribed', extract_model = NULL, extract_at = NULL WHERE guid = '<guid>'` (same pattern as **`improved_json_prune._clear_episode_topics`**). **(3)** **`bilradio import-bullets --guid <guid> --file data\transcripts_improved\<stem>.json`**. **(4)** Optional: **`prepare-improved-agent --guid <guid> --force`** to regenerate inbox Markdown, then **delete** **`data/cursor_inbox/<guid>_improve_auto_agent.md`** if the JSON was edited manually and the prompt is not needed. **(5)** **`bilradio export-github-pages`** → **commit + push** **`docs/api/bullets.json`**, **`docs/api/episodes.json`** (see **`.cursor/rules/github-pages-deploy.mdc`**, **`.cursor/rules/git-commit-push.mdc`**). Episodes touched in this workflow include **`413a1127-…`** (334), **`9ad8e4e0-…`** (326), **`a8b2b97d-…`** (324), **`944284db-…`** (Vi tester Renault/Mercedes).
...
| 2026-04-06 | **Single-episode improvement refresh** documented: clear **`topic_bullets`** / **`topic_sections`** + reset **`extract_*`**, re-**`import-bullets`**, **`prepare-improved-agent --force`** + delete inbox when unused, **`export-github-pages`** + push **`docs/api/*.json`**; sponsor-free / no meta-ad bullets; sample GUIDs **413a1127**, **9ad8e4e0**, **a8b2b97d**, **944284db** |
